...
interface OldShowsListProps {
  gotoShowView: (showId: number) => void
  gotoShowEdit: (showId: number) => void
  user?: User
}

const OldShowsList = (props: OldShowsListProps) => {
  ...
  useEffect(() => {
    setLoading(true)
    showClient.getAllPassedShows(page, 5).then((response) => {
      if (response) {
...
export default withConnect(OldShowsList)