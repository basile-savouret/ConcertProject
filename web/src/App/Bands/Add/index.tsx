...
const AddBand = (props: AddBandProps) => {
  ...
  const addBand = useCallback(
    () => {
      band && bandClient.addBand(band as Band).then((response) => {
        if (response) {
          setBand(band)
          gotoAdminActions()
        }
    ...
    <Box display="flex" justifyContent="center" alignItems="center" width="100vw" height="100vh">
      <Paper className={classes.container} elevation={5}>
        <Box display="flex" flexDirection="column" alignItems="center">
          ...
          <TextField value={band?.yearOfCreation} onChange={(event) => setBand({ ...band, yearOfCreation: Number(event.target.value) })} label="Année de création" type="number" className={classes.flexItems} />
          <Typography variant="body1" align="center">Les membres du groupe</Typography>
          <List >
            {!!band && !!band.members && band.members.map((member, index) => (
          ...
          <Button variant="outlined" onClick={addBand} className={classes.button}>Ajouter</Button>
        </Box>
      </Paper>
    </Box>
  );
...
export default withConnect(AddBand)